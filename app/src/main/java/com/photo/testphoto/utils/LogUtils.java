...
//	public static boolean sIsLogEnabled = IConfig.isLog;// 是否打开LOG
	public static boolean sIsLogEnabled = BuildConfig.DEBUG;// 是否打开LOG

	public static String TAG = "bangbang";// LOG默认TAG

	private static final String TAG_CONTENT_PRINT = "%s:%s.%s:%d";
	private static final String TAG_CONTENT_PRINT_1 = "%s:%s:%d";
	...
	private static String getContent(StackTraceElement trace) {
		String[] classNameArr=trace.getClassName().split("\\.");
		return String.format(TAG_CONTENT_PRINT_1,
				classNameArr[classNameArr.length-1], trace.getMethodName(),
				trace.getLineNumber());
	}
